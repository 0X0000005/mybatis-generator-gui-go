...
### v1.7.9.2 新增特性 (安全与配置升级)

- 🔒 **数据库存储凭证** - 移除硬编码账号密码，采用 SQLite 数据库存储账号密码，并使用 bcrypt 确保安全。
- ⚙️ **命令行动态注入** - 支持项目启动时通过 `-auth-user` 和 `-auth-pass` 动态注入或覆盖管理员账号密码。
- 👤 **在线账号管理** - 提供前端管理入口，支持用户在线修改自己的账号名和密码，提升交互体验与系统安全性。
